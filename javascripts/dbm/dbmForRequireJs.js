...
(function(aGlobalObject) {
	...
	if(dbmObject.isCreated !== true) {
		...
		dbmObject.getFileForClass = function(aClassPath) {
			...
			return aClassPath.split(".").join("/");
		};
		
		dbmObject.loadClass = function(aClassPath) {
			//console.log("dbm.loadClass");
		...
		dbmObject.classRegistered = function(aClassPath, aDependencies) {
			...
			var currentArray = aDependencies;
			var currentArrayLength = aDependencies.length;
			var pathNamesArray = new Array(currentArrayLength);
			for(var i = 0; i < currentArrayLength; i++) {
				pathNamesArray[i] = this.getFileForClass(currentArray[i]);
			}
			
			define(fileName, pathNamesArray, function() {return dbm.getClass(aClassPath)});
		};
		...
		define(["dbm/core/globalobjects/classmanager/ClassManager"], function() {return dbm;});
	}
